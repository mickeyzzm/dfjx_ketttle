SimpleEvalDialog = Ext.extend(KettleTabDialog, {
	...
	initComponent : function() {
		...
		var evaluateStore = new Ext.data.JsonStore({
			fields : [ 'value', 'text' ],
			data : [ {
				value : 'field',
				text : '将上一步结果的字段'
		...
		var wEvaluate = new Ext.form.ComboBox({
			id : 'eval',
			fieldLabel : '检验',
			anchor : '',
			flex : 1,
			...
			mode : 'local',
			triggerAction : 'all',
			value : cell.getAttribute('valuetype'),
			listeners : {
				'select' : function(value) {
					if (wEvaluate.getValue() == 'variable') {
						wSuccessWhen.setValue(cell.getAttribute('successwhenvarset'));
						...
						wSuccessWhen.setValue(false)
					}
				}
			}
		});
		
		var wFieldName = new Ext.form.TextField({
			fieldLabel : '字段名',
			anchor : '',
			flex : 1,
			value : cell.getAttribute('fieldname')
		});
		var wVariablename = new Ext.form.TextField({
			fieldLabel : '变量名',
			anchor : '',
			flex : 1,
			value : cell.getAttribute('variablename')
		});
		...
		var wFieldtype = new Ext.form.ComboBox({
			fieldLabel : '类型',
			anchor : '',
			flex : 1,
			store : typeStore,
			...
			mode : 'local',
			triggerAction : 'all',
			value : cell.getAttribute('fieldtype'),
			listeners : {
				'select' : function(value) {
		...
		var dataFormatStore = Ext.StoreMgr.get('dataFormatStore');
		var wMask = new Ext.form.ComboBox({
			fieldLabel : '掩码',
			anchor : '',
			flex : 1,
			visiable : false,
			...
			mode : 'local',
			triggerAction : 'all',
			value : cell.getAttribute('mask')
		});

		var conditionStore = Ext.StoreMgr.get('successConditionForSimp');
		...
		var wSuccessNumberCondition = new Ext.form.ComboBox({
			fieldLabel : '成功条件',
			anchor : '',
			flex : 1,
			visiable : false,
			store : Ext.StoreMgr.get('successNumberConditionStore'),
			displayField : 'text',
			valueField : 'value',
			editable : false,
			mode : 'local',
			triggerAction : 'all',
			value : cell.getAttribute('successnumbercondition')
		});

		var BooleanconditionStore = new Ext.data.JsonStore({
			fields : [ 'value', 'text' ],
			data : [ {
				value : 'true',
				text : '如果值是True'
		...
		var wSuccessBooleancondition = new Ext.form.ComboBox({
			fieldLabel : '成功条件',
			anchor : '',
			flex : 1,
			visiable : false,
			store : BooleanconditionStore,
			displayField : 'text',
			valueField : 'value',
			editable : false,
			mode : 'local',
			triggerAction : 'all',
			value : cell.getAttribute('successbooleancondition')
		});

		var wcomparevalue = new Ext.form.TextField({
			fieldLabel : '值',
			anchor : '',
			flex : 1,
			value : cell.getAttribute('comparevalue')
		});
		...
		var wSuccessWhen = new Ext.form.Checkbox({
			fieldLabel : 'Success when varible set',
			anchor : '-10',
			flex : 1,
			listeners : {
