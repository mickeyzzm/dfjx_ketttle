JobEntryEvalTableContentDialog = Ext.extend(KettleDialog, {
	...
	initComponent: function() {
		...
		var wEditbutton =new Ext.Button({xtype: 'button', text: '编辑...', handler: function() {
			...
			store.each(function(item) {
				if(item.get('name') == wConnection.getValue()) {
					var databaseDialog = new DatabaseDialog({type : 'job'});
		...
		var form = new Ext.form.FormPanel({
			...
			items: [{xtype: 'compositefield',
				...
				anchor: '0',
				items: [wSchemaname, {
					xtype: 'button', text: '浏览(B)...', handler: function() {
						me.selectSchema(wConnection, wSchemaname);
					}
				}]
			},{xtype: 'compositefield',
				fieldLabel: '目标表名称',
...
//		this.fitItems = [form];
	...
	getSQL: function(dbInfo, wCustomSQL) {
		...
		dialog.show(null, function() {
			dialog.initDatabase(dbInfo, 'job');
		});
	},
	selectSchema: function(wConnection, wSchema) {
		...
		dialog.show(null, function() {
			dialog.initDatabase(wConnection.getValue(), 'job');
		});
		return false;
	},	
