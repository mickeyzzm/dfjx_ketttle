...
public class IpFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain filterChain)
		...
		if (allowList.size() == 0 || allowList == null) {
			...
			if (!flag) {
				throw new RuntimeException("IP限制访问!");
			}
		}
	}
	...
	public void initConfig() throws IOException {
		// 将文件转化成流
		InputStream inputStream = IpFilter.class.getClassLoader().getResourceAsStream("config/ipConfig.properties");
	...
	public Boolean validate(String allowIP, String allowIPRange, String allowIPWildcard) {
		...
		// 对第二种方式进行校验
		pattern = Pattern
				.compile("(" + ipRegx + ")\\-(" + ipRegx + ")|" + "((" + ipRegx + ")\\-(" + ipRegx + ")(,|;))*");
		...
		// 对第三种方式进行校验
		pattern = Pattern.compile("(" + regx + "\\." + regx + "\\." + regx + "\\." + "\\*)|" + "(" + regx + "\\." + regx
				+ "\\." + regx + "\\." + "\\*(,|;))*");
